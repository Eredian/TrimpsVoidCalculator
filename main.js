...
var Simulator = (function () {
	...
	*/
	function Simulator(data) {
		...
		var h, currentZone, currentCell;
		var minCellsBeforeDrop, maximumVoidLevel;
		var drops, seed;
		var goldenBonus = 0;
		var totalDrops = 0;
		...
		this.run = function (loops) {
			...
			for (h = 0; h < loops; h++) {	//RUNS
				...
				cellsSinceLastDrop = lastVoidMap;
				goldenBonus = 0;
				drops = 0;
				...
				for (currentZone = 0; currentZone < z1golden; currentZone++) { // Z1 GU
					if (arrGoldenUpgrades[currentZone])
						goldenBonus += 0.02 * (currentZone + 1);
				}

				loopZones:
				for (let currentZone = 1; currentZone < targetZone; currentZone++) {	//ZONES
					if (goldenInterval != -1 && currentZone % goldenInterval == 0) {
						if (arrGoldenUpgrades[currentZone / goldenInterval + z1golden - 1]) {
							goldenBonus += 0.02 * (currentZone / goldenInterval + z1golden);
						}
					}

					if (currentZone < startZone) { continue; }

					if (_voidMaxLevel < currentZone) {
						_voidMaxLevel = currentZone;
						if ((lastPortal + 25) < currentZone)
							_voidMaxLevel = highestLevelCleared;
					}
					maximumVoidLevel = _voidMaxLevel;
					if ((maximumVoidLevel - lastPortal) < 25) {
						maximumVoidLevel = lastPortal;
					}
					if (maximumVoidLevel > 200) { maximumVoidLevel = 200; }
					minCellsBeforeDrop = (maximumVoidLevel > 80) ? (1000 + ((maximumVoidLevel - 80) * 13)) : 1000;
					minCellsBeforeDrop *= (1 - heirloomPrc);
					minCellsBeforeDrop *= (1 - goldenBonus);

					let currentCell = currentZone === startZone ? startCell : 0;
					while (currentCell < 100) {	//CELLS
						const cellIncrement = 100 - currentCell;
						let dropChance = ((Math.floor((cellsSinceLastDrop - minCellsBeforeDrop) / 10) / 50000) +
							(Math.floor((cellsSinceLastDrop - minCellsBeforeDrop + cellIncrement) / 10) / 50000)) / 2;


						if (mode === "zones" && currentZone === targetZone - 1) {	//TODO find a better solution
							if (dropsPerZone[targetZone - 1] === undefined)
								dropsPerZone[targetZone - 1] = 1;
							else
								dropsPerZone[targetZone - 1]++;
						...
						if (dropChance <= 0) {
							cellsSinceLastDrop += cellIncrement;
							currentCell += cellIncrement;
							continue;
						}
						let success = false;
						for (let i = 0; i < cellIncrement; i++) {
							cellsSinceLastDrop++;
							currentCell++;
							if (Simulator.seededRandom(seed++) < dropChance) {
								success = true;
								break;
							}
						}
						if (!success) { continue; }
						cellsSinceLastDrop = 0;
						drops++;
						...
						if (mode === "zones") {
							...
							if (zoneCount >= targetVoidMapsInRestOfRun) {
								...
								if (dropsPerZone[currentZone] === undefined)
									dropsPerZone[currentZone] = 1;
								else
									dropsPerZone[currentZone]++;

								totalZones += currentZone;
								zoneCount = 0;
						...
						var cell = ((currentZone - 1) * 100) + currentCell;
		...
		this.updateSwitches = function () {
			...
			textResultVoidMaxLevel.innerHTML = maximumVoidLevel;
			...
			*/
			...
			if (chartUpdateClock >= CHART_UPDATE_INTERVAL) {
				if (mode === "zones")
					updatePie(dropsPerZone, "Average zone until which (including) " + targetVoidMapsInRestOfRun + " Void Map" + (targetVoidMapsInRestOfRun > 1 ? "s" : "") + " will drop in rest of run");
				else if (mode === "voids")
